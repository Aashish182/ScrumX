...
import axios from "axios";
import { useSelector } from "react-redux";
import { Zap, Rocket, Clock, CheckCircle, Timer, BarChart3, ChevronRight, Target, Activity } from "lucide-react";
import SideBar from "../components/Sidebar";
import SummaryApi from "../common";
...
const CurrentSprint = () => {
    ...
    if (loading) return (
        <div className="h-screen bg-[#0b0f1a] flex flex-col items-center justify-center text-blue-500">
            <Activity className="animate-spin mb-2" size={32} />
            <div className="text-sm font-medium">Loading ScrumX...</div>
        </div>
    );

    return (
        <div className="flex min-h-screen bg-[#0b0f1a] text-slate-300">
            <SideBar />
            <main className={`flex-1 ${isOpen ? "ml-64" : "ml-20"} p-8 transition-all duration-500`}>
                ...
                {data ? (
                    <div className="max-w-5xl mx-auto">
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-start mb-8 gap-4">
                            <div>
                                <p className="text-xs font-semibold text-blue-500 uppercase mb-1">Active Sprint</p>
                                <h1 className="text-3xl font-bold text-white">{data.sprint.name}</h1>
                            </div>
                            <div className="flex items-center gap-2 bg-white/5 px-4 py-2 rounded-lg border border-white/10">
                                <div className="w-2 h-2 bg-emerald-500 rounded-full"></div>
                                <span className="text-xs font-medium text-emerald-500 uppercase">In Progress</span>
                            </div>
                        </div>

                        {/* PROGRESS CARD */}
                        <div className="bg-[#111827] border border-white/10 rounded-2xl p-8 mb-8 shadow-lg">
                            <div className="grid md:grid-cols-3 gap-8 items-center">
                                <div className="md:col-span-2">
                                    <h2 className="text-xs font-semibold text-slate-500 uppercase mb-2">Sprint Goal</h2>
                                    <p className="text-lg text-slate-200 leading-normal">
                                        {data.sprint.goal}
                                    </p>
                                </div>
                                <div className="md:text-right">
                                    <div className="text-5xl font-bold text-white">{data.metrics.progress}%</div>
                                    <div className="text-xs text-blue-500 font-semibold uppercase mt-1">Completion</div>
                                </div>
                            </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
                            <StatCard icon={<Clock size={18} className="text-blue-500" />} label="Hours" value={`${data.metrics.totalHours}h`} />
                            <StatCard icon={<Target size={18} className="text-amber-500" />} label="Remaining" value={data.metrics.remaining} />
                            <StatCard icon={<CheckCircle size={18} className="text-emerald-500" />} label="Done" value={data.metrics.done} />
                            <StatCard icon={<Zap size={18} className="text-purple-500" />} label="Priority" value="Normal" />
                        </div>

                        {/* TASKS */}
                        <div className="mb-6">
                            <h3 className="text-sm font-bold text-slate-400 uppercase">Current Tasks</h3>
                        </div>

                        <div className="space-y-3">
                            {data.subtasks.map((task) => (
                                <div key={task._id} className="group bg-[#111827] border border-white/5 hover:border-blue-500/50 p-5 rounded-xl transition-all cursor-pointer">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-4">
                                            <div className="w-10 h-10 bg-white/5 rounded-lg flex items-center justify-center text-sm font-bold text-blue-400">
                                                ID
                                            </div>
                                            <div>
                                                <h3 className="font-semibold text-slate-100 group-hover:text-blue-400 transition-colors">{task.title}</h3>
                                                <div className="flex items-center gap-4 mt-1">
                                                    <span className="text-[10px] text-slate-500 font-medium">#{task._id.slice(-6)}</span>
                                                    <span className="flex items-center gap-1 text-[10px] text-blue-500 font-medium uppercase">
                                                        <Timer size={10} /> {task.estimated_hours || 0}h
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div className="hidden sm:block">
                                                <div className="flex items-center gap-2">
                                                    <div className="w-20 h-1.5 bg-white/5 rounded-full overflow-hidden">
                                                        <div className="h-full bg-blue-600" style={{ width: `${task.percent_complete}%` }} />
                                                    </div>
                                                    <span className="text-xs font-medium text-white">{task.percent_complete}%</span>
                                                </div>
                                            </div>
                    ...
                    <div className="h-[60vh] flex flex-col items-center justify-center text-slate-600">
                        ...
                        <p className="text-sm font-medium">No active sprint found.</p>
                    </div>
                )}
            </main>
        </div>
